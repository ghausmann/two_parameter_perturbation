...
for t=1:lty
    ...
    bsss_ty(t) = compute_sss_soe_pac_dynare(M_,options_,oo_,eps_ind,guess_y);
    guessb = bsss_ty(t);
...
%--------------------------------------------------------------------------
M_.params(6) = ty_baseline; % set ty back to its baseline level
...
for t=1:lg
    ...
    M_.params(2) = mgama(t);
    bsss_g(t) = compute_sss_soe_pac_dynare(M_,options_,oo_,eps_ind,guess_g);
    guessb = bsss_g(t);
