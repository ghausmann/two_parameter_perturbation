...
parameters a, af, b, bf, betta, gama, pssi, phi, alpa, md, d0, rho_d, td, rho_y, ty, rho_q, tq, rho_eps, kappa, C0, Cs0, P0, Ps0, pf0, zSh0, zSf0, zBh0, zBf0, uy_uys_corr, uy_ud_corr, gap;
...
= zSh0*exp(zSh - pi - (gama+gap)*c)*(1 + (1-eps)*pssi*(Sh-a));
...
= zSf0*exp(zSf - pi - (gama+gap)*c)*(1 + (1-eps)*pssi*(Sf-af));
...
= zSh0*exp(zSh - pi_s - (gama-gap)*c_s)*(1 + (1-eps)*pssi*((1-Sh)-(1-a)));
