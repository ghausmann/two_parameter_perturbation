% The model generated by this script is the main application of the paper 
% "Solving Open Economy Models with Incomplete Markets by Perturbation",
% by Guillermo Hausmann-Guil (2024).
% Infinite-horizon version of the Coeurdacier-Gourinchas (2016) model.
% Two-country DSGE model with equities only.
% Includes income, redistributive, and preference shocks
% Auxiliary model uses PAC
%
% Copyright (C) 2024 Guillermo Hausmann Guil
%
% The model is defined by:
%
% Ef(yp,y,xp,x)=0
% y=g(x);
% xp=h(x)+eta*ep
% h(x)=[tilh(x);Phi(x)];
%
% The functions f and Phi are known.
% The functions g and tilh are unknown.

clear;

% Step 1: Define the model by symbolic variables.

%Parameters.
% The code approximates log-deviations of all variables (except for equities, because they can take zero values)
% Hence their deterministic steady-state values in levels enter as parameters in the model equations
syms a betta gama pssi phi alpa md d0 rho_d td rho_y ty rho_q tq rho_eps kappa C0 Cs0 P0 Ps0 pf0 zSh0 zSf0;

%States today
syms Sh Sf d d_s ey ey_s q q_s eps real
%States tomorrow
syms Shp Sfp dp d_sp eyp ey_sp qp q_sp epsp real

%Controls today
syms c c_s zSh zSf pi pi_s pf real
%Controls tomorrow
syms cp c_sp zShp zSfp pip pi_sp pfp real

% Collect State variables:
x=[Sh, Sf, ey, ey_s, d, d_s, q, q_s, eps]; % today
xp=[Shp, Sfp, eyp, ey_sp, dp, d_sp, qp, q_sp, epsp]; % tomorrow

% Collect Control variables: 
y=[c, c_s, zSh, zSf, pi, pi_s, pf]; % today
yp=[cp, c_sp, zShp, zSfp, pip, pi_sp, pfp]; % tomorrow

% Collect Parameters
symparams=[a, betta, gama, pssi, phi, alpa, md, d0, rho_y, ty, rho_d, td, rho_q, tq, rho_eps, kappa, C0, Cs0, P0, Ps0, pf0, zSh0, zSf0];

% Define the function Phi, which is the lower block of h(x).
% This block is the expected value of the exogenous state variables.
Phi = [rho_y*ey;rho_y*ey_s;rho_d*d;rho_d*d_s;rho_q*q;rho_q*q_s;rho_eps*eps];

% Define eta as in Schmitt-Grohe and Uribe (2004):
%We have 6 zero-mean shocks
eta=[zeros(2,6);ty 0 0 0 0 0;0 ty 0 0 0 0;0 0 td 0 0 0;0 0 0 td 0 0;0 0 0 0 tq 0;0 0 0 0 0 tq;0 0 0 0 0 0];

%Model equations

%budget constraint HOME
f1 = P0*C0*exp(pi+c) - ( exp(ey)*( 1 - ( exp(d0+d)/(1+exp(d0+d)) )  ) ) ...
- (Sh)*( zSh0*exp(zSh) + exp(ey)*(exp(d0+d)/(1+exp(d0+d)) )) ...
- (Sf)*( zSf0*exp(zSf) + pf0*exp(pf+ey_s)*(exp(d0+d_s)/(1+exp(d0+d_s)) ) ) ...  
+ ( zSh0*(Shp)*exp(zSh) + zSf0*(Sfp)*exp(zSf) );

%budget constraint FOREIGN
%Note that by imposing market-clearing in financial markets, we have:
%Sh_s = 1 - Sh, Sf_s = 1 - Sf, Bh_s = -Bh, Bf_s = -Bf.
f2 = Ps0*Cs0*exp(pi_s+c_s) - ( pf0*exp(ey_s+pf)*( 1 - ( exp(d0+d_s)/(1+exp(d0+d_s)) )  ) ) ...
- (1 - Sh)*( zSh0*exp(zSh) + exp(ey)*(exp(d0+d)/(1+exp(d0+d)) )) ...
- (1 - Sf)*( zSf0*exp(zSf) + pf0*exp(pf+ey_s)*(exp(d0+d_s)/(1+exp(d0+d_s)) )) ...  
+ ( zSh0*(1-Shp)*exp(zSh) + zSf0*(1-Sfp)*exp(zSf) );

%Price index HOME
f3 = (P0^(1-phi))*exp(pi*(1-phi)) ...
    - (    alpa*exp(-(1-phi)*q) + (1-alpa)*(pf0^(1-phi))*exp((pf-q_s)*(1-phi)));
%Price index FOREIGN
f4 = (Ps0^(1-phi))*exp(pi_s*(1-phi)) - ...
    ((1-alpa)*exp(-(1-phi)*q) +     alpa*(pf0^(1-phi))*exp((pf-q_s)*(1-phi)));

%Market-clearing HOME good;
f5 = alpa*((P0^phi)*C0)*exp(pi*phi + c) ...
    + (1-alpa)*((Ps0^phi)*Cs0)*exp(pi_s*phi + c_s) - exp(ey-(phi-1)*q);

%Euler equation Home equity, HOME
f6 = betta*(exp(-kappa*c))*( zSh0*exp(zShp) + exp(eyp)*( exp(d0+dp)/(1+exp(d0+dp)) ) )*exp( -pip-gama*cp ) ...
- zSh0*exp(zSh - pi - gama*c)*(1 + (1-eps)*pssi*(Shp-a));
%Euler equation foreign equity, HOME
f7 = betta*(exp(-kappa*c))*( zSf0*exp(zSfp) + pf0*exp(pfp+ey_sp)*(exp(d0+d_sp)/(1+exp(d0+d_sp)) ) )*exp( -pip-gama*cp ) ...
- zSf0*exp(zSf - pi - gama*c)*(1 + (1-eps)*pssi*(Sfp-(1-a)));

%Euler equation Home equity, FOREIGN
f8 = betta*(exp(-kappa*c_s))*( zSh0*exp(zShp) + exp(eyp)*(exp(d0+dp)/(1+exp(d0+dp)) ) )*exp( -pi_sp-gama*c_sp ) ...
- zSh0*exp(zSh - pi_s - gama*c_s)*(1 + (1-eps)*pssi*((1-Shp)-(1-a)));
%Euler equation Home equity, FOREIGN
f9 = betta*(exp(-kappa*c_s))*( zSf0*exp(zSfp) + pf0*exp(pfp+ey_sp)*(exp(d0+d_sp)/(1+exp(d0+d_sp)) ) )*exp( -pi_sp-gama*c_sp ) ...
- zSf0*exp(zSf - pi_s - gama*c_s)*(1 + (1-eps)*pssi*((1-Sfp)-a));


f=[f1;f2;f3;f4;f5;f6;f7;f8;f9]; % all conditions

% Choose approximation order

approx=3; % third order solution.

% call differentiate_dsge

model=differentiate_dsge(f,yp,y,xp,x,symparams,approx,Phi);
save('model')