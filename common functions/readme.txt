The 19 current codes in this folder are:
...
dr_gt_1_risk_linear.m: function, evaluates linear decision rules for control variables of a DSGE model solved with the algorithm by Levintal (2017).
dr_ht_1_risk_linear.m: function, evaluates linear decision rules for state variables of a DSGE model solved with the algorithm by Levintal (2017).
...
my_mvnrnd.m: function, generates samples from a multivariate normal distribution.
...
simul_linear.m: function, implements a simulation using first-order rules obtained with the algorithm by Levintal (2017).
