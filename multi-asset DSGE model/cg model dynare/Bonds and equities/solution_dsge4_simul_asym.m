...
%
% This script replicates the fourth column of Table 3 from the paper:
...
mysol = compute_sss_dsge4_asym(M_,options_,oo_,eps_ind,[a (1-a) b -b])
...
innovations = my_mvnrnd(zeros(6,1),M_.Sigma_e,(T0 + (T-1)))'; % shocks from period 2 to T
%use the Dynare matlab function simult_.m to simulate the economy:
myt =simult_(M_,options_,y0,mdr,innovations',3);
myt = myt(:,T0+1:end);
xt = myt(3:13,1:end); %states
yt = [myt(1:2,1:end);myt(14:22,1:end)]; %controls
...
my_errors =zeros(1,T);
for t=1:T
    my_errors(t) = log10(euler_errors_dsge4(P,yss,mdr,myt(:,t),epsi_nodes,weight_nodes,3));
end
